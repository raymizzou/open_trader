...
def test_order_alert_uses_only_supplied_ledger_fields() -> None:
    ...
    title, message = render_order_alert(group, broker_label="富途", trading_date="2026-07-22")

    assert title == "【需处理｜富途｜美股买入未完成｜2026-07-22】"
...
def test_fixed_feishu_title_levels() -> None:
    assert render_daily_title("富途", "美股", "2026-07-22") == "【日报｜富途｜美股趋势报告｜2026-07-22】"
...
def test_order_alert_hides_arbitrary_chinese_containing_reason() -> None:
    ...
    _, message = render_order_alert(group, broker_label="富途", trading_date="2026-07-22")
...
def test_protection_alert_hides_non_numeric_price_detail() -> None:
    _, message = render_protection_alert(
        "富途", "美股", "HIG", last_price="133.90", active_line="/Users/ray/secret"
    )
...
def test_protection_alert_normalizes_symbol_and_hides_unsafe_title_detail() -> None:
    title, _ = render_protection_alert(
        "富途", "美股", "b1", last_price="133.90", active_line="134.1650"
    )
    ...
    assert unsafe_title == "【紧急｜富途｜美股保护线触发｜未知标的】"
