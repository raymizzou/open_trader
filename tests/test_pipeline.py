...
def test_tiger_long_term_strategy_cli_parser_contract() -> None:
...
def test_tiger_long_term_strategy_cli_closes_quote_client(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    monkeypatch.setattr(cli, "generate_tiger_long_term_strategy", fake_generate)

    exit_code = cli.main([
        "run-tiger-long-term-strategy",
        "--date", "2026-07-14",
        "--config", str(tmp_path / "strategy.json"),
        "--data-dir", str(tmp_path / "data"),
        "--dry-run",
    ])

    assert exit_code == 0
    assert captured["generate_kwargs"] == {"update_latest": False}
    assert captured["generate_args"][3] is captured["client"]
    assert captured["client"].closed is True
    output = capsys.readouterr().out
    assert "status: shadow" in output
    assert "members: 8" in output
    assert "eligible: 5" in output
