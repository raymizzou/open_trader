...
def test_trend_review_sync_stats_parser_exposes_real_api_workflow() -> None:
    ...
    assert not hasattr(args, "tiger_config_dir")
    assert not hasattr(args, "tiger_account")
...
def test_trend_review_sync_stats_main_wires_only_selected_cn_simulation(
    ...
    class FakeActual:
        def __init__(self, **_: object) -> None:
            raise AssertionError("CN cycle must not initialize the Futu actual client")
    ...
    monkeypatch.setattr(cli_module, "FutuActualFillClient", FakeActual)
    monkeypatch.setattr(
        cli_module,
    ...
    assert cycle_calls[0]["actual_client"] is None
    output = capsys.readouterr().out
    assert "status: completed" in output
    assert "statistics_cutoff_at: 2026-08-08T15:00:00+08:00" in output


def test_trend_review_sync_stats_us_initializes_futu_simulation_and_futu_actual(
    ...
    class FakeActual:
        def __init__(self, **kwargs: object) -> None:
            created.append(f"actual:{kwargs['trd_market']}")
    ...
    assert result == 0
    assert created == ["US", "actual:US"]
    assert cycle_calls[0]["actual_client"] is not None
