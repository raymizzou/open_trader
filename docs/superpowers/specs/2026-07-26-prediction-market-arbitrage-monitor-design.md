...
| `UI-14 Monitored event disclosure` | Open the prediction-market workspace, expand one monitored event, allow a watcher poll, then collapse it and poll again. | Every event is collapsed on initial render; the selected event remains expanded after refresh; after manual collapse it remains collapsed after refresh. | Browser assertions exercise the real state fetch and identify disclosures by stable event key. | Auto-expanding the first event, reopening a collapsed event, collapsing an expanded event during polling, or preserving state by list position when an event ID exists. |
