...
### 8.2.1 已确认的页面导航与候选展开方式

`预测市场` 保持为 Open Trader 一级导航。进入后使用二级策略切换：

```text
YES/NO套利 | LLM对冲套利
```

- `YES/NO套利` 是现有同 condition 的 YES + NO 页面，也是生产默认页；
- `LLM对冲套利` 是本设计的跨 condition 阈值关系页面；
- 点击后原地切换，不刷新页面；
- 选择只保留在当前页面会话，不写本地存储或服务端；
- Polymarket 健康、钱包、地区、余额与执行状态共用；
- 两种策略的扫描指标、候选、历史参考和日志分别展示。

LLM 对冲候选使用原生 `<details>`，默认全部折叠。摘要必须直接展示：

- 候选名称和缩略 condition IDs；
- `APPROVE`、`REJECT` 或不可用状态；
- 24 小时成交量；
- 含费最大成本和最低赔付；
- 理论最低利润；
- 简单年化。

点击简单年化所在的候选摘要后，候选内部展开：

1. 当前最大成本、最低赔付、最低利润、预计剩余占用天数；
2. `最低利润 / 最大成本` 的简单收益率；
3. `简单收益率 * 365 / 剩余天数` 的简单年化；
4. 当前、7 天和 30 天历史同类参考；
5. 两个市场规则、condition IDs、买入腿和确定性证明；
6. Codex 结构化结论、中文摘要、reason codes、证据和不确定性；
7. 仅在完整通过时出现的人工确认入口。

`annualized_yield` 在领域层是小数比例，Dashboard 必须乘以 `100` 后再加 `%`。例如领域值 `0.2155` 显示为 `21.5%`，不得显示为 `0.22%`。结算时间缺失时，展开区显示“不可计算”，不得推测剩余天数。

扫描日志继续使用原生 `<details>` 并默认折叠。年化分布放在候选展开区，不能与摘要中的当前简单年化并列成两个互相竞争的主指标。
