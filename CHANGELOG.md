...
- Issue 148: LP preparation transient failures now keep bounded 5/10/20/30-minute retries, probe the affected dependency every 60 seconds subject to a longer server `Retry-After`, isolate failed markets, and coalesce a five-minute automatic-recovery alert with independent delivery retry. Recovery notices require complete fresh coverage of the affected scope; short faults stay silent, acknowledged episodes can retry failed recovery delivery, and operator blockers remain paused. Runtime scheduling wakes on the earliest probe or notification deadline while risk ticks continue independently. Dashboard recovery labels and the LP manual SOP now describe the automatic path. Verified by the focused Docker recovery, SDK-boundary, runtime, and Dashboard selectors; no live calls, orders, deployment, or push were performed.
