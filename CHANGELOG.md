...
- 修复预测市场状态接口因 24 小时 Codex 用量逐行载入而超时的问题：用量改为 SQLite 单行聚合并缓存一分钟，Dashboard 读取缓存中文标题不再写入 cache-hit 事件；保留既有审计数据与真实 Codex 调用统计。使用现有 702 万条用量记录验证状态接口恢复 HTTP 200，相关回归 465 个通过。
