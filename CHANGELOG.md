...
- 「当天 LP 委托」表在 LP 收益率与份额占比之间新增「今日奖励(累计 · $/小时)」列（按标的显示市场当日累计已赚奖励与当前每小时奖励估算，与结算卡 market_rewards 同源、计奖日 UTC 当日累计；读不到显示 UNKNOWN 并保留上次值/上次成功时间、绝不按 0 计，空态 colspan 6→7），后端看板奖励缓存覆盖面从手动单市场扩到委托表当天全部市场（复用既有 60 秒节流与后台批量读线程）；评审修复：读失败保留值只显示 UNKNOWN＋上次成功时间（不把旧值当新值渲染绿色），订单表列宽由 6 列重排为 7 列。验证：`make test TEST='tests/test_prediction_service.py'` 167 通过、`make test TEST='tests/test_dashboard_web.py'` 545 通过（rebase 后复跑），新增 A1–A3/B1–B4 用例均先红后绿。未做 push 或部署。
- #181：LP「官方竞争」列升级为密度筛选 + 轻中两档保留 + 竞争值持久化回退（polymarket_lp_views.py + polymarket_lp.py + prediction_arbitrage_store.py + dashboard.js/.css）。官方 `market_competitiveness` 直接当密度用（上游已含奖池归一，不二次相除）：只保留 轻 [1,30) / 中 [30,300) 两档市场，零竞争（既有危险信号排除）、过薄 <1、重度 ≥300、无数据四类全部整行排除并计数（原因码 competition_empty/competition_too_thin/competition_too_crowded/competition_missing）；无冷启动守卫——缓存与库皆空即一律 competition_missing 排除（空表是预期行为）。竞争值每轮全量持久化：服务新增 `lp_market_competitiveness` 表（幂等建表）与 `lp_competitiveness_upsert/map/count`，`_refresh_competition` 成功轮批量落库（含零值；失败仅 WARNING 不影响刷新），投影 `_competition_entries` 合并库回退——缓存新鲜（3 小时窗内）条目 `source="fresh"`，缺失或超龄回取库值 `source="store"`（带库内 checked_at），两处皆无则不出现；`LP_COMPETITION_MAX_AGE` 1→3 小时（定案 A：全量 17.7 万市场单轮 ~44 分钟、周期 ~104 分钟，3 小时窗让「旧」只在真断流时出现），views 不再做年龄门（store 值不论多旧照用，stale 仅作信息位），竞争并列 tie-break 语义不变。刷新节奏 600→3600 秒（密度是慢变量）。前端：列名改「官方竞争 · 密度」，轻/中行加绿/琥珀 pill（`.lp-tier-light`/`.lp-tier-mid`，配色对齐 mock 与既有 token），store 来源行副行「旧 · {HKT 时间}」；空态与副标题补「重度竞争 X · 无数据 Y · 过薄 Z」，新增观测行「竞争数据：最近一轮成功 {时间|UNKNOWN} · 覆盖 N · 轻 N · 中 N」（funnel 新键 competition_light/competition_mid/competition_round_checked_at）。阈值定稿（原占位 0.5/3/12）：全量分布采样（16,258 个活跃有奖池市场）锚定分位数——过薄界 1≈p10（竞争流动性不足一天奖池）、轻上界 30≈中位数、中上界 300≈p90。验证：新增 S1–S5（存储往返/落库/库回退/密度分档/无守卫，阈值一律 monkeypatch 不依赖占位数）与 W1（pill、旧值副行、计数、观测行）；五套定向 `make test`（store 133、lp 200、dashboard_web 538、service 164、runtime 69）全绿。未做 push 或部署。
- 生产冒烟浏览器用例修复（test-only）：`production-smoke.spec.ts` 两处 `.pm-lp-card` 可见性断言在活动 LP 会话下触发 Playwright 严格模式冲突（resolved to 2 elements）——#166 起页面存在外层 LP 大面板与内嵌组卡两层同名 `.pm-lp-card`，有活动会话即必失败，为 #178 部署后连续两次 ROLLBACK 的根因（与 #178 代码无关）。修法：两处改锚外层面板 `.first()`（外层面板在 DOM 中先于内嵌卡片），断言意图不变（LP 面板在 LP 标签页可见）；第 68 行多腿套利页 `toHaveCount(0)` 语义正确、不动。验证：对线上 8766 只读执行 `playwright test production-smoke.spec.ts --project=chromium`（当前线上恰有活动会话、正是暴露该问题的数据状态），1 passed。未做 push 或部署。
- #178：LP 开仓/加量提交撞全局执行锁改 3 秒有界等待 + busy toast 文案改准（prediction_arbitrage_execution.py + dashboard.js）。成因：全局执行锁 `execution.lock` 被每 1 秒一轮的 lp_tick 网络读取长期占用（实测约 78% 时间），而 `lp_submit_entry`/`lp_submit_augment` 是「单发非阻塞抢锁、抢不到立刻 busy 拒绝」——有活动组后用户几乎下不了第二单，且前端 busy toast 一律误报「业务忙：已有活动 LP 会话」。修法：`_acquire_global_lock` 主体抽成单次尝试 `_acquire_global_lock_once`，新增仅关键字参数 `wait_seconds`（默认 0.0=单次尝试，与其余 9 个调用点行为逐字节等价、零改动）；两提交入口改传模块常量 `_LP_SUBMIT_LOCK_WAIT_SECONDS = 3.0`（0.05 秒间隔重试、`time.monotonic()` 截止），等满窗口仍抢不到才返回与今天一字不差的 `{"state": "busy", "reason": "execution_lock"}` 并打一行含实际等待秒数的 WARNING；门禁顺序不变——幂等重放 → 熔断 → 同标的冲突 → active_execution → 等锁 → 锁内双查 → 提交，便宜拒绝先于等待。前端 `lpSubmitToastCopy` busy 分支保留 `lp_session_market_active` 特判，其余 reason 主行复用既有 `lpSubmitStateMessage` 准确句子（如 execution_lock →「已有另一笔操作正在确认，请稍后重新确认。」），副行「稍后再次确认即可 · 本单未发出」，退役误报句「业务忙：已有活动 LP 会话」；lp_tick/lp_stop/旧两阶段端点/10 秒冷却/四色 toast 语义零改动。验证：新增 A1（占锁 0.1 秒后释放、0.5 秒窗内提交走完 entry_open，旧实现立刻 busy 先红）、A2（等满 0.3 秒返回体整字典相等 + 耗时 ≥ 窗口 + WARNING 含实际秒数）、A3（无争抢零等待成功且锁随即可再获取）、A4（占锁期间同标的冲突仍 <1 秒即拒，钉死便宜拒绝先于等待）、A5（加量镜像 A1）与 W1–W4（execution_lock/active_execution/未知 reason 文案矩阵、market_active 特判一字不差回归、退役句全 busy 输出清零）；`make test TEST='tests/test_prediction_arbitrage_execution.py'`、`tests/test_dashboard_web.py` 两套全绿。未做 push 或部署。
- #177：LP 官方竞争缓存翻页中断修复（polymarket_trading.py 竞争读单路径重写 + polymarket_lp.py 书签存取）。成因：竞争翻页走 SDK `secure_clob.get_json`（read 超时固定 10 秒、不支持按次传超时），`/rewards/markets/multi` 单页偶发超时即整轮异常报废、下一轮又从第一页重读，列表后段活跃市场永远翻不到，看板「官方竞争」列整列未知。修法：翻页改裸 `urllib` 通道（与价格历史同款 `Request`+`User-Agent: OpenTrader/1.0`+`Accept: application/json`+按次 `timeout=15` 秒，opener 仍走构造器既有 `urlopen_fn` 注入口）；每页最多试 3 次、两次间隔 1 秒（等待可被 stop_event 打断），3 次全挂 `logger.warning`（含游标、已成功页数、错误）后按既有 partial/unknown 语义收场、不再静默吞轮；中断轮把最后成功页的 `next_cursor` 作为 `resume_cursor` 返回，LP 服务存为内存书签 `next_start_cursor`，下一轮以 `start_cursor` 续翻（走完 `LTE=` 自动清空；游标越界时接口回 0 行+`LTE=` 自然收束）。返回契约既有键（state/complete/checked_at/round_checked_at/competitiveness/not_updated）与语义全部保留，仅新增 `resume_cursor`/`next_start_cursor`；从书签续翻的完整轮对书签之前的条目 setdefault 保留 previous 值与旧时间戳，从第一页起步的完整轮仍整体替换；看板投影、1 小时过期窗、600 秒刷新节奏、「竞争=0 排除」语义、prediction_runtime 线程一律未动；书签仅内存不落库。验证：新增 S1–S6（两页夹具：每页请求 timeout 恰 15.0、第 2 页首败重试成功且恰 2 次请求、持久失败恰 3 次请求+`resume_cursor=="Mg=="`+该模块一条 WARNING、`start_cursor="Mg=="` 首请求即续翻共 1 次请求且 condition-a 保旧时间戳、stop 打断重试等待无重试且不抛异常、全挂轮 previous 原样保真）与 L1（服务侧书签落状态、次轮回传、走完清空）；`make test TEST='tests/test_polymarket_trading.py'`、`tests/test_polymarket_lp.py`、`tests/test_prediction_runtime.py` 三套全绿。全量首轮 test_prediction_service.py::test_lp_scan_does_not_block_maintenance 因假 reader 旧签名（缺 start_cursor、参数绑定 TypeError 且被刷新路径吞掉）失败，已补齐 test_prediction_service.py 2 处 + test_polymarket_lp.py 7 处共 9 个假件签名（其中多处在吞异常下已成死代码、一并复活），修复后 tests/test_prediction_service.py 164 通过、tests/test_polymarket_lp.py 197 通过。部署后线上发现默认 urllib 通道走 macOS 系统代理（本机 127.0.0.1:1082）致翻页中途滴流卡死（栈卡 SSL read、线程第一轮即挂、缓存为空），R2 改为无代理直连 opener（`build_opener(ProxyHandler({}))`，恢复旧 httpx 通道的直连路线），新增默认通道绕过系统代理用例。未做 push 或部署。
- #176：LP 提交路径盘口新鲜度取时倒置修复（纯服务端 polymarket_lp.py 五处取时点位移）。成因：submit_entry/submit_augment/augment_preview/augment/start confirm 在校验前取时钟 `now`，而 `book.received_at` 由行情读取完成时打戳，`年龄 = now − received_at` 恒为负，`_freshness`（age<0 即拒）确定性拒 `book_freshness_stale`（生产实测 15:27:54 取 now、15:27:57 收到盘口，负 3 秒被拒）。修法：五处统一改为「快照读取之后取 now」，读后 now 继续传给 `_validate_snapshot`/`_fresh_candidate_row`/`expiration_for_review` 与执行入口；start confirm 原读前取 now + best_bid_minimum 分支读后条件重取，改为读前 `read_started` 仅作 `_read_candidate_snapshot` 死参数、读后无条件重取 `now`；读前另有的合法时钟使用（preview 过期检查等直调 `self._now()`）原样保留；`_freshness`/`_read_snapshot`/`_read_candidate_snapshot`/preview/`_reconcile_session`（tick 走另一套新鲜读助手）一律未动，负年龄防御语义保留。验证：新增 A1（推进时钟夹具：读完成前 clock=T0、读后 T0+3s、received_at=T0+3s，先红 `rejected/book_freshness_stale` 后绿 `entry_open`+恰一单）、A2（submit_augment 同款推进时钟，加量空闲档 0.28 成功+恰一张新 post，直接绿）、A3（读后时钟前进 70 秒、received_at=T0+3s，年龄 67>60 上限仍拒 `book_freshness_stale` 且零挂单——修复≠删检查）；`make test TEST='tests/test_polymarket_lp.py'` 全文件 196 通过。未做 live 调用、push 或部署。
- #174：LP 挂单提交体身份同源化（纯前端 dashboard.js + 测试夹具对齐生产形状）。成因：候选行顶层 `token_id`/`outcome` 是入队评估的那条腿，而看板展示与下单弹窗取的是 `selected_direction`（最优腿）；生产候选池过半行两腿不同，此前提交体混搭「outcome=展示所选腿 + token_id=行顶层评估腿」，服务端 #163 身份校验必拒 `market_identity_mismatch`。修法：`lpOrderIntent` 新增捕获 `token`（取 `selected_direction.token_id`，与 `outcome` 同源），`POST /lp/orders` 提交体 `token_id` 改取该值（不回退行顶层、不加空值兜底——行顶层无 token 的畸形行按既有契约由服务端拒 `token_id_invalid`）；market_id/condition_id、价格、数量、review_at、estimated_target_quantity、candidate_policy 取值全部不变，请求体四要素（标的/方向/价格/数量）自此与界面所见同源。服务端校验零改动。测试夹具对齐：共享 `candidateRow.selected_direction` 补 `token_id` 字段（生产投影该层必带同腿 token_id，旧形状生产不存在；该行顶层 token_id 同值，仍是同腿行），W2 期望体一字未改。验证：新增 A1（两腿错位行：提交体 token_id/outcome 均取 selected_direction，先红 `token-fed-no != token-fed-yes` 后绿）与 A2（同腿行回归，直接绿）；`make test TEST='tests/test_dashboard_web.py'` 全文件 533 通过（7 项 browser 标记用例按既有约定未选）。未做 live 调用、push 或部署。
- #167：LP 同组多价位下单与分价位位置保护。同一活动组内支持多价位挂单（如 YES 0.42 与 YES 0.40 并存，每张单保留原价）：`POST /lp/augment` 增加可选 `price`（缺省=组价，`price_invalid`/`price_level_active`/`price_above_best_bid` 按序校验，状态门禁沿用 `_LP_AUGMENT_STATE_BLOCKS`），同价补量语义退役（该价位仍有自有非终态挂单即拒，先撤该价再追加），新价位经既有资金校验（全组占用口径）后为该价建独立保护桶；`queue_protection` 载荷升级 v2 `{version, data_failures, levels}`（组级断联计数、按价位分桶建档/评估/撤单，桶基线=该价档位深度，`estimate_lp_queue_position` 复用），读路径三档兼容（无键=未启用、老标量包成单桶继续跑、残缺桶仅置 unknown），首次写回自然落新形状；任一价位成交（组级 buy_filled>0，买入合计已按组聚合入场+加量）立即撤组内全部自有 BUY（复用 `_cancel_owned_orders`，桶标 group_fill_collect 随回执收敛、不补买不重买），成本/回款/剩余/$5 止损照旧按组合并核算且止损证据一次 latch；组级断联 10 次保守撤整组（逐桶 book_unreliable）；复核截止沿用组首单不顺延，到期一次收掉全部自有单（桶标 review_deadline）；保护通知按价位各发一条（标题带价位，如「YES 0.42 位置保护已触发撤单」），随组收单/到期收尾不发触发通知；同键重放仍只产一单、首单买一锚定与 submit_entry/候选列表零改动。看板：加量按钮改名「追加」并解锁价位输入（默认组价可改，弹窗带价位规则提示），保护副行从组级一条改每桶一条（价位标签 `.qp-price` 开头，随组收单/到期文案独立），组卡新增价位明细块（价/量/剩余/保护 pill/A 估算），新拒因文案入矩阵，10 秒锁/toast 四色/幂等键铸造不变。验证：定向全量绿——tests/test_polymarket_lp.py 192 通过、tests/test_prediction_service.py 164 通过、tests/test_dashboard_web.py 531 通过、tests/test_prediction_arbitrage_execution.py 313 通过、tests/test_prediction_runtime.py 69 通过、tests/test_prediction_arbitrage_store.py 132 通过；store 表结构与索引零变化。未做 live 调用、push 或部署。
- #166：LP 多标的多组并存。不同标的可同时各建一个 LP 活动组（同标的+方向仍只许一组，存储唯一性改为按 condition_id+outcome 的表达式唯一索引，零数据迁移），每组独立监控、独立 $5 止损、独立复核时间、独立退出；资金校验在全部提交/预检点统一改按「扣掉全部活动组委托占用」后的可用余额/授权复核（占用不可得按 account_facts_unknown 拒绝，理由 balance_insufficient 语义不变）；加量收紧为只许 entry_open 组（passive_exit/entry_submit_pending 分别拒 session_passive_exit/session_entry_pending，新老两条加量路径共用同一张拦截表）；同标的+方向再开组的拒绝理由统一为 lp_session_market_active（载荷带冲突组 session_id，服务门禁与数据库兜底同映射）；tick() 改聚合报文（单组顶层形状不变并附 sessions 键，多组顶层为最差态聚合+各组完整载荷+最旧核对时间，单组核对异常不吞其他组）；日报触发改为每组都就绪才触发（截止撤销等待例外按组保留）；refresh_rewards 按组点名刷新；stop(None) 在两组及以上拒绝 session_ambiguous（需点名会话），status(None) 仍回最新活动组；看板快照新增 lp_sessions 组视图列表（最新在前，无活动组放最近完结组沉底），lp_session 键保留=最新活动组，当天委托/持仓按 token 集合归因各组不塌缩；前端会话详情区渲染多张组卡（多组标题「活动组 · N」），加量按钮只出现在 entry_open 组的行，busy 文案改「该标的已有活动组，本单未提交；换一个标的即可开仓」。修复环补充：同标的双方向在两条提交路径（#163 submit_entry 与老两段式 start confirm）同样被风险层参与检查挡住——账户事实出现同 condition 任一方向的挂单/持仓即拒 market_already_participating（与老 preview 同理由同语义；幂等重放先于该检查，加量路径不套用），完整同标的双方向仍由 #168 验收开放。N_LEG↔LP 全局互锁、熔断、全局锁、账户级互斥与 #163 的 10 秒入口锁机制未变。验证：定向五文件全绿——tests/test_polymarket_lp.py 180 通过、tests/test_prediction_arbitrage_execution.py 312 通过、tests/test_prediction_service.py 163 通过、tests/test_dashboard_web.py 529 通过（7 项 browser 标记用例按既有约定未选）、tests/test_prediction_runtime.py 68 通过，另 tests/test_prediction_arbitrage_store.py 132 通过；未做 live 调用、push 或部署。

## 2026-09-21

- #163：LP 首开/加量改为「一次确认即提交」（代交 #164 加量链路）。LP 下单与加量弹窗单相化：填写与查看阶段零服务端调用（不再有「预检并预览」一步），点「确认提交」弹窗立即关闭、LP 卡片内出现「正在提交」toast、挂单与加量两个入口共享锁死 10 秒；服务端新增 `POST /api/prediction-arbitrage/lp/orders` 与 `POST /api/prediction-arbitrage/lp/augment` 两个端点（熔断/互斥/全局锁保护下恰好读一次新鲜行情、做一次完整校验，然后挂一张 post-only BUY；加量点名 session_id，成功后新单并入既有位置保护、复核截止沿用首单不重算），结果以 HTTP 200 + body.state 返回，前端按 state 用 toast 呈现（成功绿/拒绝红/结果未知蓝，30 秒自动淡出，最多 3 条）。默认试挂单锚定提交时买一：确认价与新鲜买一不一致直接拒绝（不改价、不改量、不追价）；5%/自定义模式不锚定。幂等键在确认点击时铸造（UUID），同键重放服务端只产生一张单；10 秒无回执报「结果未知」（不宣称 失败/成功/已撤销）并解锁，提前回执只更新自己的 toast、不提前解锁，迟到回执按键归属绝不覆盖新意图。本地价格/数量格式异常不发请求、不锁入口。旧两段式端点（/lp/preview、/lp/sessions、/lp/augment/preview、/lp/sessions/{id}/augment）原样保留，UI 不再调用。验证：三套定向测试全绿——`tests/test_polymarket_lp.py` 147 通过（新增 lp163 服务面 13 用例：一次提交恰一次快照读/一单、买一锚定拒、自定义偏移、同键重放、交易所拒单落库重放、回执未知不重发、余额不足无会话行、活动会话 busy、复核过近、加量追加/基线重锚/截止沿用、点名会话绑定、受阻状态矩阵、加量幂等与未知）；`tests/test_prediction_service.py` 162 通过（新增 lp163 路由 5 用例：鉴权先行、严格 schema 400、熔断/busy/买一漂移均 200+state、成功路径、加量路由契约）；`tests/test_dashboard_web.py` 528 通过（新增 `_LP163_INTERACTIVE` harness 捕获 setTimeout + crypto 存根与 W1–W9 用例：零调用直至确认、确认单次 POST+10 秒锁、固定 10 秒解锁、超时未知、文案矩阵、迟到回执只改自己、新意图新键、加量单次提交共享锁、加量超时重键）。存量 #158 T4/T5/T6/T15f（断言已删除的预检相/preview_id 确认路径）按授权删除由 W 系替代；T7/A5/A8/A9 断言弹窗内失败态与预检体的部分按 toast/单次提交新契约改写（语义不弱化）。未做 live 调用、下单、push 或部署。
- #165：LP 显式订单与组归属准备重构（行为保持，为 #166 多组并存打地基，本票不开放多组）。tick 拆分：组级监控/对账逻辑逐语句搬入新函数 `_reconcile_session`，tick() 遍历活动会话逐组对账并返回最后一组汇报（无活动会话仍返回 `{"state": "none", "session_id": None}`）；store 新增复数选择器 `lp_active_sessions()`（非终态、created_at 倒序，schema 与唯一索引 `one_active_lp_session` 零改动）；预留、准备排序、首见豁免改按会话列表并集（单活动组取值与改前一致）；LP 看板 open orders / 当天委托 / 缓存成交行新增 `session_id` 归属键（手动与首见 episode 行保持 null，顶层 payload 不加新键），完结会话行继续走 active→latest 回退标 `system_managed`；加量按钮渲染 `data-session-id` 且点击仅在 session_id 与服务端会话一致时弹窗。status/stop 传不存在 session_id 仍返回 none 载荷、绝不回退活动组。验证：新增 10 个定向用例（store 复数选择器、tick 无会话护栏、终态短路免写、未知/历史 session_id 护栏、today 行归属与回退护栏、HTTP 投影透传、按钮 data-session-id 与点击门控、预留条目），另更新 `tests/test_prediction_runtime.py` 两个既有精确契约用例补 `session_id` 键；`make test TEST=tests/test_polymarket_lp.py` 全绿（139 通过）及 store/execution/service/dashboard/runtime 各 lp 定向套件全绿。首轮 Candidate Acceptance 暴露无 `session_id` 键载荷下 managed 归因回归（视图构造误设门槛），修复（managed 派生恢复无条件、行归属仍按 session_id）后重跑 **Candidate Acceptance: PASS（8897 通过）**；rebase 到 main a37d01b7（issue 162 合入，CHANGELOG 位置性冲突已解、dashboard.js 与测试自动合并）后复跑 **Candidate Acceptance: PASS**。未跑 host-readiness / production-smoke（属部署链条），未做 live 调用、push 或部署。

- #162：LP 下单/加量/撤单弹窗生命周期与数值保真修复（纯前端 dashboard.js）。其一，5 秒后台场所轮询在 N_LEG 暂停或请求失败时不再无差别关闭 LP 弹窗——轮询失效路径只关 N_LEG 域弹窗（nleg_order/nleg_incident/reset/allowance_cleanup，order 维持豁免），lp_order/lp_augment/lp_cancel 弹窗与已填价量在转圈/暂停期间保持存活；`invalidatePredictionNLegReads()` 本体不再关窗。其二，模态生命周期加 epoch 世代校验：打开/关闭各自递增，每个异步响应（预检/提交/撤单等）先校验世代与 kind，弹窗已被关闭或替换的迟到响应整包静默丢弃（不写 data、不渲染、不写 error/摘要、不改 busy）；唯一例外是迟到的确认/撤单成功（订单确已落库）仍写「已登记/已加量/撤成」摘要并重拉 LP 看板报信，但绝不碰当前弹窗；`predictionModalHtml` 的兜底分支改为仅 kind="order" 渲染，空/未知 kind 返回空串（消灭迟到响应渲染出的「确认真实下单」空白框）；click 处理器新增动作→kind 映射守卫，跨弹窗类型的动作（如在 lp_order 下触发加量确认）不发任何请求。其三，数量全原样字符串：lp_order/lp_augment 的 5% 目标量不再经 formatDisplayNumber 千位逗号+两位舍入，输入框、预检/提交请求体、选项标签、复核 echo、合并预估「新增」一律原样（1500.125 不再变 1,500.13、不再 NaN 致 UNKNOWN）；「预计占用」是唯一计算值，初始渲染、改价、改量三时机都按 `$ + (price×qty).toFixed(2)` 现算（如 0.42×1500.125 → $630.05），空缺显示 -。行为变化点：离开 prediction 视图（setWorkspaceView）显式关闭全部弹窗 kind（含 order）；lp→multi_leg 切签关闭 LP 三类弹窗（multi_leg→lp 维持失效+N_LEG 域关闭）；LP 三类弹窗 busy 转圈期间「取消」按钮与 Esc 保持可用（关闭即作废迟到响应），其它 kind 维持 busy 不可关。验证：`make test TEST='tests/test_dashboard_web.py'` 全绿（516 通过，7 deselected 为既有浏览器标记），LP 面 `-k "lp"` 77 通过全绿；新增 `_LP162_INTERACTIVE` 交互 harness（可延迟 fetch + 受控轮询）与 A1–A10 验收用例逐条先红后绿（A1/A2/A6/A7/A3 工作树红，A4/A8 等基线代码探针红），既有 #158 T3/T4/T15f 等用例零修改。未做 live 调用、下单、push 或部署。
...
- 「当天 LP 委托」表三项前端改动：完结沉底——组内任一委托仍有在挂余量的活跃组排前、全部委托无在挂余量的完结组沉底并整行淡化（td 与链接降为 muted 色），两区各自沿用已知收益率降序 → condition_id 字典序，完结组不因收益率上浮，表头副标题同步改为「活跃在前 · 已完结沉底 · 各按当前小时奖励率降序」；成交状态三色 pill（复用现有 pm-pill / status-partial / status-ok token）——组级显示 挂单中 · 已成交 N（中性）/ 部分成交 N/M（琥珀）/ 完全成交 N/N（绿）/ 已完结 · 未成交（中性），逐笔明细追加「已成交」（绿）、「已成交 N」（琥珀）、「挂单中」（中性）着色 tag 且既有文字全部保留；新增「实际占用资金」列（位于份额占比与压力损失之间），observed 口径读 `lp_observations[conditionId].occupied_capital_usd`（持仓均成本 + 在挂买单），观测缺失、字段 null 或账户事实过期一律显示 UNKNOWN 并注明原因，绝不填 0，空态 colspan 5→6。后端 Python 零改动。验证：`make test TEST='tests/test_dashboard_web.py'` 全绿（493 通过）；新增 3 个用例（完结沉底排序、状态 pill 与淡化、占用资金列 UNKNOWN 口径）先红后绿，既有用例仅按简报对齐 表头数组两处与空态 colspan 一处。未做 live 调用、push 或部署。
- Issue 159: 网页手动单首见基线兜底保护上线（#152 提交基线路径零改动，纯增量）。用户直接在 polymarket.com 网页人工挂的 BUY 单，由 10 秒账户观察循环以「相邻两轮成功观察 diff」判新（进程首轮与距上一轮成功观察超 30 秒只建底不判新；断档期间挂的单位置未知，用户手动撤），对非会话 token、无活动保护段的新 BUY 在 LP 服务读盘口登记「首见基线」（每 token 各自一段：锚=最早首见 BUY，并列按 venue created_at、再按 order_id；锚价=锚价位；baseline_front=首见盘口档位总量−该价位自己 BUY 余量和；登记延迟如实入 payload，无任何时间上限），登记失败进内存 pending 每轮重试、观察链断/进程重启/订单终态即永久位置未知；1 秒 LP 监控 tick 对每段与 #152 同构评估（保守口径复用 `estimate_lp_queue_position` 原函数），A ≤ 50% 自动撤同 token 同锚价全部自己 BUY（intent 每目标先 lp_actions pending 再逐张撤、身份复核、撤单时点余量 first-write-wins 持久化、ordered union 记账），行情连续失效 10 次保守撤（book_unreliable），熔断/只读/身份冲突/锚点审计会话创建失败（如 n-leg 批期占用插入门）→ blocked 一次性通知、恢复后再受阻重通知一次，回执收敛 已撤/部分成交，锚单全部终态段终态、token 释放可开新段；成功/受阻通知复用 #152 模板并带「首见基线」标识，投递走既有 `set_protection_notifier` 接线（runtime 零改动）。Dashboard：首见段所在 token 行挂保护 summary（baseline_source=first_observation，会话 summary 补 baseline_source=submit 纯增量字段），锚行「首见」来源 chip、副行新增键盘可聚焦徽标「首见 · 估算假设」及逐字 tooltip（说明兜底路径固有偏差），非锚行 chip 仍「手动」、登记 chip 仍「登记」，六态 pill 与数据行文案逐字复用现有模板。存储新增 `lp_first_seen_episodes` 表 + 每 token 单活动段部分唯一索引（user_version 15，expand-only）。验证：定向组 tests/test_polymarket_lp_risk.py、tests/test_polymarket_lp.py、tests/test_prediction_arbitrage_execution.py、tests/test_prediction_service.py、tests/test_dashboard_web.py 合计 1116 通过全绿（rebase 到 ed11197e 后，含其 3 个并行改动用例）；P1/D1/D2 先红后绿，E1–E4（数字原点 baseline_front=8,000、A=80%/66.67%/50%×2）、E5 范围与 10 次断联保守撤、E6 撤单闭环（逐目标 intent+回执、部分成交如实记账、不重复撤不扩量、一次性「首见基线」通知）、E7 受阻零写调用与恢复重通知、C1 会话并存不开段、C2 双 token 独立触发互不串扰、T 终态释放可开新段、S fake-SDK 端到端（登记→触发→撤单→投影）与 dashboard.js 渲染断言全过。未做 live 调用、下单、push 或部署。
- Issue 158: LP Dashboard 手动下单入口与委托加量上线。候选表操作列改为「挂单」按钮（移除 Polymarket 链接按钮，标题仍为链接），点击打开 lp_order 确认模态：试挂单/5% 单/自定义三情形（试挂与 5% 价量锁定只读，仅自定义可改价改量并继承当前值；5% 目标量 UNKNOWN 时选项禁用并标注原因），预检走既有 `POST lp/preview`（试挂模式自动附 `candidate_policy:"best_bid_minimum"`），第二阶段展示服务端事实与初始 A 预估——空档位提示「提交后立即触发位置保护撤单」、A ≤ 50% 警示「提交后首个监控 tick 即触发位置保护撤单」、大于 50% 正常监控；确认提交 `{preview_id, idempotency_key}`（幂等键开模态时生成一次，双击与网络异常重试同键，服务端返既有会话不重复下单）。提交时刻买一校验对所有 lp/sessions start 生效：预检凭证 best_bid 与新鲜盘口不一致即拒 `best_bid_changed`（幂等去重先于该检查）；`preview_expired/preview_consumed/candidate_changed` 显示原因并提供「重新预检」（同幂等键、新 preview_id）；busy/locked 按定案文案在模态内呈现（LP 提交类响应统一 HTTP 200 由 body state 分支渲染）。当天 LP 委托的系统会话行新增「加量」按钮（纯网页手动单行不渲染）：lp_augment 模态按会话入场价锁定（默认加 5% 单/自定义数量，自定义实时按合并口径 front/(front+own+qty) 显示加后 A），走新端点 `POST lp/augment/preview` + `POST lp/sessions/<id>/augment`（严格 `{session_id, quantity}` 与 `{preview_id, idempotency_key}` schema、生产鉴权同既有 LP 路由），加量单登记进会话（augment_order_ids + augment-submit 审计）并与入场单同保护伞——加量时保护基线按合并口径重锚（档位−既有 BUY 余量），复核/stop 时入场单+加量单一并请求撤、零成交平仓后会话 complete；#152 保护状态机与阈值不动。候选行新增 review_at 投影（下次北京 08:00）供下单模态原样透传；空态文案补充「挂单」引导；修复候选表 8 列定宽（原 1-6 列合计 100% + fixed 布局把 7/8 列压成零宽）。验证：`make test` 定向组 tests/test_polymarket_lp.py（123 通过）、tests/test_prediction_service.py（156 通过）、tests/test_prediction_arbitrage_execution.py（298 通过）、tests/test_dashboard_web.py（501 通过）全绿；T1/T5/T6/T7/T10–T17/T2/T3/T4/T8/T14f/T15f/T16f 验收用例先红后绿（T17 红暴露加量未重锚保护基线导致运行时误触发，已修复）。未做 live 调用、下单、push 或部署；Candidate Acceptance 由主代理负责。
- Issue 158: LP Dashboard 手动下单入口与委托加量上线。候选表操作列改为「挂单」按钮（移除 Polymarket 链接按钮，标题仍为链接），点击打开 lp_order 确认模态：试挂单/5% 单/自定义三情形（试挂与 5% 价量锁定只读，仅自定义可改价改量并继承当前值；5% 目标量 UNKNOWN 时选项禁用并标注原因），预检走既有 `POST lp/preview`（试挂模式自动附 `candidate_policy:"best_bid_minimum"`），第二阶段展示服务端事实与初始 A 预估——空档位提示「提交后立即触发位置保护撤单」、A ≤ 50% 警示「提交后首个监控 tick 即触发位置保护撤单」、大于 50% 正常监控；确认提交 `{preview_id, idempotency_key}`（幂等键开模态时生成一次，双击与网络异常重试同键，服务端返既有会话不重复下单）。提交时刻买一校验对所有 lp/sessions start 生效：预检凭证 best_bid 与新鲜盘口不一致即拒 `best_bid_changed`（幂等去重先于该检查）；`preview_expired/preview_consumed/candidate_changed` 显示原因并提供「重新预检」（同幂等键、新 preview_id）；busy/locked 按定案文案在模态内呈现（LP 提交类响应统一 HTTP 200 由 body state 分支渲染）。当天 LP 委托的系统会话行新增「加量」按钮（纯网页手动单行不渲染）：lp_augment 模态按会话入场价锁定（默认加 5% 单/自定义数量，自定义实时按合并口径 front/(front+own+qty) 显示加后 A），走新端点 `POST lp/augment/preview` + `POST lp/sessions/<id>/augment`（严格 `{session_id, quantity}` 与 `{preview_id, idempotency_key}` schema、生产鉴权同既有 LP 路由），加量单登记进会话（augment_order_ids + augment-submit 审计）并与入场单同保护伞——加量时保护基线按合并口径重锚（档位−既有 BUY 余量），复核/stop 时入场单+加量单一并请求撤、零成交平仓后会话 complete；#152 保护状态机与阈值不动。候选行新增 review_at 投影（下次北京 08:00）供下单模态原样透传；空态文案补充「挂单」引导；修复候选表 8 列定宽（原 1-6 列合计 100% + fixed 布局把 7/8 列压成零宽）。生产修复：加量「加 5%」默认量不再依赖候选池——候选漏斗对已有委托的市场打 known_participation 并排除，会话激活后标的不在候选池导致 5% 选项大多 UNKNOWN 禁用；现 lp_order 预检体在行内有值时原样附带 informational-only 字段 `estimated_target_quantity`（随会话 payload 存储、lp_status 投影透出，无值投影 null），加量模态默认量取值顺序改为 会话值 → 候选池保底 → 均无则禁用+UNKNOWN；该字段仅作 UI 默认量，不参与任何校验与下单数值（加量量仍全部经 augment_preview/augment 校验）。验证：`make test` 定向组 tests/test_polymarket_lp.py（123 通过）、tests/test_prediction_service.py（156 通过）、tests/test_prediction_arbitrage_execution.py（298 通过）、tests/test_dashboard_web.py（501 通过）全绿；T1/T5/T6/T7/T10–T17/T2/T3/T4/T8/T14f/T15f/T16f 验收用例先红后绿（T17 红暴露加量未重锚保护基线导致运行时误触发，已修复）。未做 live 调用、下单、push 或部署；Candidate Acceptance 由主代理负责。
- Issue 158: LP Dashboard 手动下单入口与委托加量上线。候选表操作列改为「挂单」按钮（移除 Polymarket 链接按钮，标题仍为链接），点击打开 lp_order 确认模态：试挂单/5% 单/自定义三情形（试挂与 5% 价量锁定只读，仅自定义可改价改量并继承当前值；5% 目标量 UNKNOWN 时选项禁用并标注原因），预检走既有 `POST lp/preview`（试挂模式自动附 `candidate_policy:"best_bid_minimum"`），第二阶段展示服务端事实与初始 A 预估——空档位提示「提交后立即触发位置保护撤单」、A ≤ 50% 警示「提交后首个监控 tick 即触发位置保护撤单」、大于 50% 正常监控；确认提交 `{preview_id, idempotency_key}`（幂等键开模态时生成一次，双击与网络异常重试同键，服务端返既有会话不重复下单）。提交时刻买一校验对所有 lp/sessions start 生效：预检凭证 best_bid 与新鲜盘口不一致即拒 `best_bid_changed`（幂等去重先于该检查）；`preview_expired/preview_consumed/candidate_changed` 显示原因并提供「重新预检」（同幂等键、新 preview_id）；busy/locked 按定案文案在模态内呈现（LP 提交类响应统一 HTTP 200 由 body state 分支渲染）。当天 LP 委托的系统会话行新增「加量」按钮（纯网页手动单行不渲染）：lp_augment 模态按会话入场价锁定（默认加 5% 单/自定义数量，自定义实时按合并口径 front/(front+own+qty) 显示加后 A），走新端点 `POST lp/augment/preview` + `POST lp/sessions/<id>/augment`（严格 `{session_id, quantity}` 与 `{preview_id, idempotency_key}` schema、生产鉴权同既有 LP 路由），加量单登记进会话（augment_order_ids + augment-submit 审计）并与入场单同保护伞——加量时保护基线按合并口径重锚（档位−既有 BUY 余量），复核/stop 时入场单+加量单一并请求撤、零成交平仓后会话 complete；#152 保护状态机与阈值不动。候选行新增 review_at 投影（下次北京 08:00）供下单模态原样透传；空态文案补充「挂单」引导；修复候选表 8 列定宽（原 1-6 列合计 100% + fixed 布局把 7/8 列压成零宽）。生产修复：加量「加 5%」默认量不再依赖候选池——候选漏斗对已有委托的市场打 known_participation 并排除，会话激活后标的不在候选池导致 5% 选项大多 UNKNOWN 禁用；现 lp_order 预检体在行内有值时原样附带 informational-only 字段 `estimated_target_quantity`（随会话 payload 存储、lp_status 投影透出，无值投影 null），加量模态默认量取值顺序改为 会话值 → 候选池保底 → 均无则禁用+UNKNOWN；该字段仅作 UI 默认量，不参与任何校验与下单数值（加量量仍全部经 augment_preview/augment 校验）。验证：`make test` 定向组 tests/test_polymarket_lp.py（123 通过）、tests/test_prediction_service.py（156 通过）、tests/test_prediction_arbitrage_execution.py（298 通过）、tests/test_dashboard_web.py（501 通过）全绿；T1/T5/T6/T7/T10–T17/T2/T3/T4/T8/T14f/T15f/T16f 验收用例先红后绿（T17 红暴露加量未重锚保护基线导致运行时误触发，已修复）。评审修复：lp_order/lp_augment 确认的 HTTP 200 body state 分支补齐两个非成功终态——`entry_rejected`（交易所拒单、未产生委托）模态内错误条「未登记：交易所拒绝了订单，未产生委托；请重新预检后再试」（不关模态、不写摘要，lp_order 走既有「重新预检」路径）、`needs_attention`（提交结果未知）模态内警示条「提交结果未知（回执未确认），请刷新看板核对会话状态后再操作；不要重复提交」；成功收尾改为显式判定「body 携带 session_id 且 state 非失败态」，幂等重放返既有活动会话（review/passive_exit/stop_loss_exit/complete 等）仍按「已登记（返既有会话）」收尾，busy/locked/rejected 既有文案与幂等键逻辑不动。验证：`make test TEST='tests/test_dashboard_web.py'`（501 通过）与 `make test TEST='tests/test_prediction_service.py'`（156 通过）全绿；T7 扩展与 T15f 侧补用例先红后绿。未做 live 调用、下单、push 或部署；Candidate Acceptance 由主代理负责。
- Issue 158: LP Dashboard 手动下单入口与委托加量上线。候选表操作列改为「挂单」按钮（移除 Polymarket 链接按钮，标题仍为链接），点击打开 lp_order 确认模态：试挂单/5% 单/自定义三情形（试挂与 5% 价量锁定只读，仅自定义可改价改量并继承当前值；5% 目标量 UNKNOWN 时选项禁用并标注原因），预检走既有 `POST lp/preview`（试挂模式自动附 `candidate_policy:"best_bid_minimum"`），第二阶段展示服务端事实与初始 A 预估——空档位提示「提交后立即触发位置保护撤单」、A ≤ 50% 警示「提交后首个监控 tick 即触发位置保护撤单」、大于 50% 正常监控；确认提交 `{preview_id, idempotency_key}`（幂等键开模态时生成一次，双击与网络异常重试同键，服务端返既有会话不重复下单）。提交时刻买一校验对所有 lp/sessions start 生效：预检凭证 best_bid 与新鲜盘口不一致即拒 `best_bid_changed`（幂等去重先于该检查）；`preview_expired/preview_consumed/candidate_changed` 显示原因并提供「重新预检」（同幂等键、新 preview_id）；busy/locked 按定案文案在模态内呈现（LP 提交类响应统一 HTTP 200 由 body state 分支渲染）。当天 LP 委托的系统会话行新增「加量」按钮（纯网页手动单行不渲染）：lp_augment 模态按会话入场价锁定（默认加 5% 单/自定义数量，自定义实时按合并口径 front/(front+own+qty) 显示加后 A），走新端点 `POST lp/augment/preview` + `POST lp/sessions/<id>/augment`（严格 `{session_id, quantity}` 与 `{preview_id, idempotency_key}` schema、生产鉴权同既有 LP 路由），加量单登记进会话（augment_order_ids + augment-submit 审计）并与入场单同保护伞——加量时保护基线按合并口径重锚（档位−既有 BUY 余量），复核/stop 时入场单+加量单一并请求撤、零成交平仓后会话 complete；#152 保护状态机与阈值不动。候选行新增 review_at 投影（下次北京 08:00）供下单模态原样透传；空态文案补充「挂单」引导；修复候选表 8 列定宽（原 1-6 列合计 100% + fixed 布局把 7/8 列压成零宽）。生产修复：加量「加 5%」默认量不再依赖候选池——候选漏斗对已有委托的市场打 known_participation 并排除，会话激活后标的不在候选池导致 5% 选项大多 UNKNOWN 禁用；现 lp_order 预检体在行内有值时原样附带 informational-only 字段 `estimated_target_quantity`（随会话 payload 存储、lp_status 投影透出，无值投影 null），加量模态默认量取值顺序改为 会话值 → 候选池保底 → 均无则禁用+UNKNOWN；该字段仅作 UI 默认量，不参与任何校验与下单数值（加量量仍全部经 augment_preview/augment 校验）。验证：`make test` 定向组 tests/test_polymarket_lp.py（123 通过）、tests/test_prediction_service.py（156 通过）、tests/test_prediction_arbitrage_execution.py（298 通过）、tests/test_dashboard_web.py（501 通过）全绿；T1/T5/T6/T7/T10–T17/T2/T3/T4/T8/T14f/T15f/T16f 验收用例先红后绿（T17 红暴露加量未重锚保护基线导致运行时误触发，已修复）。评审修复：lp_order/lp_augment 确认的 HTTP 200 body state 分支补齐两个非成功终态——`entry_rejected`（交易所拒单、未产生委托）模态内错误条「未登记：交易所拒绝了订单，未产生委托；请关闭后重新发起下单」（不关模态、不写摘要；复审修正——被拒会话已按幂等键落库，同键「重新预检」必返同一结果，故不再提供「重新预检」出路、该结果模态内也不出现「重新预检」字样，lp_augment 防御侧对称同改，预检类拒绝的重新预检路径不变）、`needs_attention`（提交结果未知）模态内警示条「提交结果未知（回执未确认），请刷新看板核对会话状态后再操作；不要重复提交」；成功收尾改为显式判定「body 携带 session_id 且 state 非失败态」，幂等重放返既有活动会话（review/passive_exit/stop_loss_exit/complete 等）仍按「已登记（返既有会话）」收尾，busy/locked/rejected 既有文案与幂等键逻辑不动。验证：`make test TEST='tests/test_dashboard_web.py'`（501 通过）与 `make test TEST='tests/test_prediction_service.py'`（156 通过）全绿；T7 扩展与 T15f 侧补用例先红后绿。复审修正（entry_rejected 去死路）：`make test TEST='tests/test_dashboard_web.py'`（501 通过）全绿，T7 entry_rejected 用例按新契约更新先红后绿。未做 live 调用、下单、push 或部署；Candidate Acceptance 由主代理负责。
...
- LP 位置保护通知现在显示市场标题、链接、BUY 方向、受影响订单及撤单时余量；成功通知补充按份额估计的排队说明、首次挂单与最终撤单确认时间和跨日存续时长，受阻通知明确失败原因且不宣称已撤。首见基线与提交基线均持久化订单时间，旧段只从本地缓存补全身份；缺失事实保持未知并带完整诊断 ID。回执收敛记录本地确认观察时间，撤单被熔断阻止期间，受阻通知只发一次，首见段恢复后重新受阻才再次提醒。评审修复：回执仅全为成交或其他终态时不再误报撤单成功；撤单受阻且身份与数量事实齐全时显示实际残余份数；dashboard orders/positions 保留原有契约并移除多余 created_at，paused-runtime 测试在既有时限内等待公开 dashboard reward 状态。验证：Docker 三项公开接口定向测试3通过、1 warning；Candidate Acceptance 尚待复验；未进行 live 调用、下单、push 或部署。

## 2026-09-20

- Issue 152: LP BUY 队列位置保护上线。经预览/确认入口提交的 LP entry BUY 在提交边界登记同价位盘口基线（校验已保证该 token 无既有挂单、档位全为前方；档位缺失登记 0 不拦单，基线快照同步落盘口样本档），运行时在既有每秒 LP 监控 tick 内估算 A 比例（保守口径：下单后同价位任何减少均计入前方，不区分成交与撤单；回执缺失、档位缺失、数据矛盾按 UNKNOWN 处理不猜数值），A ≤ 50%（恰好 50% 也触发）即自动撤掉同 token 同价位的自己全部 BUY（登记单+手动单逐张核对身份后逐张撤，不同价位不碰），先记保护专用审计 action 再撤、禁止复用手动撤单管线；只撤余量、不补买、不自动创建/修改退出卖单、不重挂追价，回执终态收敛为 已撤/部分成交。行情断联（快照/盘口未知）连续 10 秒按一次新账户读取保守撤单；熔断期不发撤单、如实上报受阻；回执未确认下一 tick 重试且不重复撤。preview 返回提交后初始 A 预计（不拒绝不加门槛）；`lp_status` 透出保护基线/最新评估/状态机/撤单范围全字段；「当天 LP 委托」BUY 行加 登记/手动 来源 chip，组级新增保护副行（状态 pill：监控中/撤单中/已撤/部分成交/未知/撤单受阻；数据行：前方≈/同价位 · A%（触发线 50%）· 触发撤同价 N 张 · 数据时间；无锚手动组显示「位置未知 · 尚未建立位置保护」，卖出组显示「不适用（卖出）」，前方≈ 带保守估算悬浮说明）；触发撤单与受阻经飞书（成功/受阻模板）+ xiaoai 语音短句双通道各通知一次，重启不重发、受阻后恢复成功补发一次。验证：`make test` 定向组 tests/test_polymarket_lp_risk.py（32 通过）、tests/test_polymarket_lp.py（86 通过）、tests/test_prediction_arbitrage_execution.py（298 通过）、tests/test_prediction_service.py（154 通过）、tests/test_dashboard_web.py（487 通过）全绿，T1–T26 验收用例先红后绿；未做 live 调用、下单、push 或部署。评审修复：断联计数与保守撤单改为复用保护运行时同一 entry 存活门控（已成交/已请求撤/终态/无单号即复位计数且不触发），撤销过时价位上无关挂单的缺陷消除；部分失败的撤单 episode 改为跨重试累计（cancel_targets/canceled_order_ids 保持全量并集、撤单余量按撤单时点各自余量累加），成功通知仅在全量目标回执撤销后发一次且按全量口径报告，部分成交收敛不再把成交量冒充撤单余量；dashboard 保护副行 ratio 缺失显示「A UNKNOWN（触发线 50%）」不再虚构「0% ≤ 50% 已触发」，已撤/部分成交终态的张数与余量改读持久化字段、不再从在挂行推导（新增 R1–R8 回归用例先红后绿；定向组 lp 93、lp_risk 32、execution 298、prediction_service 154、dashboard_web 488 全绿）。复审修复：撤单时点余量改为在撤单请求（意图/pending）阶段按当期 rows 逐张持久化（cancel_target_remaining，取不到记 UNKNOWN 不记 0），canceled_remaining 口径改为对全量已撤集合（含回执证明已撤但撤单未获确认的订单）按持久化值求和、任一 UNKNOWN 即整体 UNKNOWN，通知与 dashboard 不再虚构「合计余量 0 份」或部分求和冒充全量（dashboard canceled_remaining=null 显示 UNKNOWN；新增 R9/R10 回归用例先红后绿；定向组 lp 95、lp_risk 32、execution 298、prediction_service 154、dashboard_web 489 全绿）。


- Issue 157: LP 收益率榜单从「整轮扫描」改为「持续滚动候选池」。探索线程每次调用处理一批（≤10 市场、≤20 token 一次盘口读，批间隔下限 2 秒），按基础筛选队列轮转：从未尝试的市场优先、其余按上次尝试时间升序，失败按市场计 60/120/300 秒退避；无 300 秒窗口、50 市场上限与 60 秒 attempted 门槛，竞争数据改由独立线程每 600 秒刷内存缓存（批路径只读缓存、永不阻塞）。有效候选池按 condition_id 存成功估值行，每行 5 分钟有效期、按当前时钟现算到期（读取与发布两路过滤，重启恢复保留原 updated_at/expires_at，读取不续期）；重估三分法：合格且收益率变化→替换并按新值降位，读取失败/数据不足→保留旧行至原到期、标「刷新失败」；确定性拒绝→立即出池回轮转队尾。发布防覆盖只用一行级写保护（新估值判定时间 ≥ 行内 updated_at 才覆盖），不做在途合并与版本号。排名键改为 收益率降序 → 估值时间降序 → condition_id 兜底（竞争值与占资退出排名键），recommendations=[榜首]、榜首退出由下一名自动补位（删除「head 不可用不递补」）。维护线程保留 30 秒提前量整表续命框架，发布改写池行；落库节流 ≥5 秒，内容=池行+事件冷却确认+轮转调度戳。快照删除 60 秒整体过期语义；`funnel.reasons` 结构不变、整轮计数器换持续计数器；Dashboard 删除「本轮扫描…X/50」「停止原因」等旧字串，新增候选池持续状态行（有效/待探索/刷新失败）与第 7 列「估值时间+有效期倒计时+状态徽章」（刷新失败行琥珀徽章「刷新失败 · 保留至原到期」）。评审修复（第一轮）：竞争刷新线程补上生产 runtime 布线（`_start_candidate_competition_monitor`，随既有候选线程启动/停止）；删除维护路径调试打印；`refresh_candidates` 批中途异常改为诚实失败发布（清 scanning、retention_reason=candidate_refresh_failed、保留池行、记日志）；失败标记与拒绝出池纳入行级时间守卫（晚到失败/拒绝不再错标或删除较新行，守卫拒绝的 success 不再误计维护失败退避）；每次发布顺带全量淘汰过期池行并连带清理对应 qualification facts（持久化载荷与重启恢复不再堆积幽灵行）；`candidate_pending_count` 改按当前队列成员直接计数未尝试市场（已退出队列市场的轮转戳不再掩盖未试市场）；停止原因改按发布携带本批语义（健康批次清除陈旧 stop_reason/retention_reason）。验证：Docker 全量 Candidate Acceptance PASS（8831 通过、5 skipped，含 lp 113、views 22、risk 32、runtime 62、dashboard_web 490、preparation_partial 20、prediction_service 155、prediction_arbitrage_execution 298、lp_auto_recovery 33 定向组全绿；新增 A1–A12/B1 验收用例先红后绿，旧契约用例按新契约逐个修正期望；评审修复 R1–R7 各带先红后绿回归用例，R2 删除打印经全 diff 检查确认无残留）。未进行 live 调用、下单、push 或部署。
...
- Issue 138 (round 2): LP 候选排序改为「目标 5% 官方奖励份额的预计每小时收益率」降序，并修复当前奖励率 native/sponsored 重复计奖。`lp_reward_rates` 的池值改由定向 public selected catalog（每 condition 基础 N / 合计 T 各一次读取，复用并发上限 4）推导，小时奖励 =(N×native份额% + S×sponsored份额%)/100/24（S=T−N），认证载荷两源的 `rate_per_day` 不再相加；正数分量缺份额、池冲突或读取失败按 reason 降级 UNKNOWN，单市场失败不牵连兄弟市场。新增纯函数 `estimate_lp_target_share_yield`（累计深度中点同资格口径、二次奖励距离权重、竞争上界 C=min(A,B)+|A−B|/3、解 q·w/3÷(C+q·w/3)=5% 得目标量，向上取整 0.01 份；UNKNOWN 不填 0、不回退奖池上限）。候选扫描去掉「凑满 10 个通过即停」，检查满 50 个市场或队列耗尽才停再选最佳 10 个（停止原因只剩 checked_limit/queue_exhausted/account_unavailable）；YES/NO 方向按预计收益率更高者选择（平局 token 兜底）；发布行新增 预计收益率/小时、目标量、目标占资、预计小时毛奖励 与估值状态/时间字段，移除 `realtime_query_rate_upper_bound`（视图内部排队键保留，不再展示）。60 秒维护改为对本轮全部 ≤10 发布行等权刷新：一次批量盘口读取（≤20 token）重跑资格与估值并全表重排，当前推荐徽章跟重排后第 1 名；单行失败保留旧估值标「本轮未更新」排已刷新行之后，整轮失败全表降级仅供阅读；资格 60 秒内翻转如实降级展示不隐藏行。Dashboard 候选表第 4 列改为「5% 奖励份额 · 预计收益率/小时」（副行 目标占资/最小试挂分列，缺值显示待测），表底副注改新停止语义；当天 LP 委托表不改、自然吃到去重值。验证：定向测试组 trading/lp_risk/lp/lp_views/prediction_service/prediction_runtime/dashboard_web/lp_preparation_partial 全绿；全程只读外部市场数据，无挂单/撤单/改价/加量，未做 live 调用、push 或部署。
- LP 手动撤单上线:「当天 LP 委托」页的逐笔撤单、标的级撤全部、表级撤全部按钮全部启用,均经确认模态触发,页脚说明改为「撤单即时生效;已成交部分不可撤」。新增 `POST /api/prediction-arbitrage/lp/orders/cancel`(三选一互斥选择器 + confirm=true),执行层按最新账户读解析目标(仅当前快照可见的 LP 单,非 LP 单与系统托管单一视同仁)、逐笔呈报 撤销/未撤/跳过,成功后刷新看板;部分失败与账户读失败发飞书通知并持久错误条,撤单结果写入 manual 审计会话。验证:`make test` 全量通过(新增 trading 原语、执行层、HTTP 端点、前端渲染共 27 个先红后绿用例(含评审修复轮 5 个回归用例));未做 live 撤单、部署或 push。

- Issue 148: LP preparation transient failures now keep bounded 5/10/20/30-minute retries, probe the affected dependency every 60 seconds subject to a longer server `Retry-After`, isolate failed markets, and coalesce a five-minute automatic-recovery alert with independent delivery retry. Partial catalog/read interruptions remain recoverable, failed-market full reads retain their per-market backoff across restart, partial progress does not close an unresolved fault episode, and stale generation results cannot overwrite a recovered market. Recovery notices require complete fresh coverage of the affected scope; short faults stay silent, acknowledged episodes can retry failed recovery delivery, and operator blockers remain paused. Runtime scheduling wakes on the earliest probe or notification deadline while risk ticks continue independently. Dashboard recovery labels, manual recovery fixtures, warm-restart timing, and shadow seeding lifetime checks now cover the same contracts. Verified by the scoped 400-test Docker regression; no live calls, orders, deployment, or push were performed.

- Issue 147: Dashboard LP candidates now show the selected plan's direction, hide failed or plan-less rows from the candidate table while retaining their reasons in diagnostics, and keep valid expired snapshots read-only. Verified the focused display regressions and the full `tests/test_dashboard_web.py` file; backend timing and recovery behavior were not changed.

- Issue 145:「当天 LP 委托」表改为一标的一行（按 condition_id 合并多笔委托），列序调整为 标的 → LP 收益率(推荐 → 实际) → 份额占比 → 压力损失(警戒线 10%) → 委托与成交量，全表左对齐；LP 收益率格改为「推荐（试挂基准）→ 实际（当前）」并移除加单空间 pill 与基准时间戳行；份额占比格新增官方奖励份额 + 状态 pill 与盘口份额副行（我的同侧挂单余量 ÷ 盘口同侧总量，观察值新增 book_shares 字段，盘口缺失显示 UNKNOWN 不按 0 计）；委托与成交量列显示合计（统一价 × 合计 / 合计 / 买卖拆分）与逐笔明细。撤单/撤全部按钮为纯 UI 预留位（disabled，无事件无路由，本期未启用）。份额预警全量开启：删除逐市场勾选、`/api/prediction-arbitrage/lp/share-watch` 路由与 `set_lp_share_watch`，监控集合改由未完成挂单推导（无挂单自动停止并清计时），告警语义不变（>8% 持续 60 秒、每 episode 一次、夜间静音、UNKNOWN 保持旧值）。SOP 第 2.1 节同步改为全量开启描述。验证：`make test TEST='tests/test_prediction_service.py'`（136 通过）、`make test TEST='tests/test_dashboard_web.py'`（477 通过）、`make test TEST='tests/test_polymarket_lp_risk.py'`（17 通过）全绿；新增 A1–A8/B1–B5/C1 验收用例先红后绿。未进行 live 调用、下单或部署。

- Issue 146 implementation: Polymarket LP 三条循环重新协调。LP 页面 5 秒轮询改读后台快照（新增 10 秒一轮的快照线程与 `refresh_lp_dashboard_snapshot` 管线；首访缓存为空时返回 `state:"snapshot_pending"`，页面显示「快照准备中」提示，不做同步首填），页面读取不再触发任何外部账户读取。候选全量扫描与当前推荐维护拆为两个独立线程：扫描成功轮后等 300 秒、失败轮等 60 秒，手动刷新只唤醒扫描线程并强制新轮；维护按「最旧数据源年龄 ≥30 秒或任一源过期」提前刷新（60 秒有效期与 `_candidate_source_expired` 不变），删除 60 秒发布年龄与维护 attempted_at 门槛，全部调度等待下限 1.0 秒、上限 300 秒（消除 50ms 空转）。维护失败改按尝试完成时刻起算 60/120/300 秒退避（≥3 连败恒 300 秒），成功即复位，失败时推荐清空但 head 行保留恢复资格；新扫描发布成功同样复位退避。新增代际保护：维护结果发布前复查代际，扫描发布的新 head 不会被旧的维护结果覆盖；扫描失败回退发布改以发布时刻最新快照为基底，不会回滚维护成果。trading client 新增 `lp_account_snapshot_shared`（单飞行 + 10 秒 TTL），扫描/维护/快照三方共享账户读取，原 `lp_account_snapshot` 及调用方零改动。快照顶层新增 `maintenance_diagnostics`（started_at/finished_at 与 account/metadata/reward/books 各段实际耗时）、`maintenance_next_attempt_at`、`maintenance_consecutive_failures` 诊断字段。`candidate_maintenance_wait_seconds` 重写（退避余量或最旧源 30 秒提前量，全路径钳制 [1.0, 300.0]，无 head/无 facts/无源返回 None，坏时间戳/未来戳返回下限）。重启后退避与代际从零开始（不持久化，报告中已说明）。Docker 验证：新增 A1 提前量、A4 退避与等待下限、A2 同类不重叠、A3 长扫描不阻塞维护、A5 代际保护、A6 仪表盘快照、A7 共享账户读取、A8 调度独立性、A9 诊断字段、`_candidate_source_expired` 59/61 秒边界与前端快照准备中横梁测试；受契约影响的既有用例按新契约修正期望（60 秒发布年龄门槛 → 30 秒提前量、维护失败由「永久失败标志」改为 60 秒退避重试、`lp_dashboard()` 由逐次全管线改为读后台快照），`make test` 定向组 lp/prediction_service/prediction_runtime/trading/dashboard_web 全绿。未做 live 调用、下单、push 或部署。

## 2026-09-19

- Issue 143 implementation: LP 候选刷新从「每轮仅队首实时核验」改为分批验证并向后补位。每批 ≤10 个不同市场（9 正常 + 1 备用交错、condition 去重、备用每轮保底 1 席），整批一次盘口读取（≤20 token）、两方向统一资格判定后合并，凑满 10 个通过／检查满 50 个市场（100 token）／队列耗尽即停；账户事实不可用时轮首早停（已检查记 0、不烧预算、保留上一轮快照）。候选表只展示实时通过者：按实际占资重算乐观上限降序（竞争→占资→token 兜底），第 1 名为「当前推荐」并独享 60 秒维护（维护结果原位刷新表内首行，后续行保持检查时快照），行龄超 300 秒降级「仅供阅读」；新增 300 秒完整扫描最小间隔（force=False 窗口内零外部读取，手动/页面 force 立即新轮，runtime 60 秒唤醒不重复扫描），漏斗新增扫描覆盖计数与停止原因（filled/checked_limit/queue_exhausted/account_unavailable），preview 复核一次读取所选市场两向 token 且与扫描预算分离、不替换已确认请求身份。Dashboard 按已批 Mock 重做候选区：标题「待试挂候选」、扫描进度行、8 列表头、当前推荐/本轮通过徽章与降级行。准备管道部分失败场景（test_lp_preparation_partial.py，20 用例）按 passers-only 语义同步修复：夹具补齐 #142/#143 事实要素（fees_checked_at、账户完整性、退出流动性第二档深度），跨时钟跳跃场景改为断言扫描覆盖记账与停止原因。评审修复：备用通过者的「假设上限/小时」列改按实际占资重算值展示（删除 queue=backup 的 UNKNOWN 特判）、60 秒维护失败的首行改降级「维护失败 · 仅供阅读」徽章、维护刷新首行占资时同步用与扫描共用的同一公式重算假设上限、批扫描的资格事实改为轮末在状态锁下整体引用替换（消除与面板快照 deepcopy 的并发竞态）、漏斗「待测候选」阶段在批扫描完成后改报发布通过者数使漏斗/进度行/表格三处一致。Docker 定向回归（lp_views/lp/lp_risk/trading/prediction_service/prediction_runtime/dashboard_web/lp_preparation_partial）全绿；修复后定向复跑（lp/lp_views/prediction_service/prediction_runtime/dashboard_web 738 通过 + lp_preparation_partial 20 通过）全绿。生产缺陷修复（第 2 轮）：真实时钟下轮长超过 60 秒事实窗口时，后批共享事实（metadata/fees、奖励目录、账户）仍按轮首快照判定，全部误判 `market_metadata_stale` 未知（生产首轮 checked=50、passed=0、rejected=10、unknown=40 全为该码）；小时级准备缓存还会让首批即过期。现按 #142「过期更新一次、重新判定」定案：每批判定前以本批判定时钟对批内市场共享事实做年龄检查（≤60 秒直接复用），过期类别分别用 `lp_market_metadata_fresh`（仅本批 condition）、定向 `lp_reward_catalog`、`lp_account_snapshot` 更新一次，并提取轮首同构的共享方向构造（`_lp_direction_fact`，不复制公式）重建方向事实；刷新失败以旧事实诚实降级为 stale 未知、本轮不重试、下一轮 force 恢复；账户续期沿用 `_account_after_reservations` 口径不重复扣减预留，队列仍按轮首预算保持不变。推进时钟用例 R2-1～R2-4 先红后绿；preview 复核、runtime 部分准备、partial 重试暂停、metadata 重试派发等 5 个既有用例因该已批行为更新期望（旧期望把「过期→未知→不发布」固定为正确结果）。生产缺陷修复（第 3 轮）：每批判定时钟改在共享事实续期读取完成之后捕获（续期过期检测仍用续期前批时钟、签名不变），消除续期读取网络时延使新戳晚于判定钟、`evaluate_lp_entry` 负年龄守卫把刚续期事实全部误判 stale 未知的生产缺陷；新增「续期读有时延」推进时钟用例先红后绿，受影响四文件组（lp/prediction_service/prediction_runtime/lp_preparation_partial）复跑 273 通过。Docker 定向组复跑 892 通过（1 跳过为 Keychain 环境限制）。未进行 live 调用、下单或部署。
...
- 修复 `trend-drawdown-unlock` 对 allocation 时代策略的解析缺陷：官方人工解锁命令
  ...
  事件与 rebase 语义不变。
- 趋势报告时序重排：CN（东方财富）/HK（辉立）趋势报告改为不早于当天
  21:00 生成（此前分配敲定后约 16:20–17:45 即生成），让晚间 19:00 开始的
  持仓录入能被当天 21:00 的报告自然采用，消除 lineage 落后一天的问题；
  CN/HK 的「Trend Animals 数据仍未就绪」失败截止从 19:00 放宽到 22:00，
