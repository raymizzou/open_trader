...
- LP 位置保护通知现在显示市场标题、链接、BUY 方向、受影响订单及撤单时余量；成功通知补充按份额估计的排队说明、首次挂单与最终撤单确认时间和跨日存续时长，受阻通知明确失败原因且不宣称已撤。首见基线与提交基线均持久化订单时间，旧段只从本地缓存补全身份；缺失事实保持未知并带完整诊断 ID。回执收敛记录本地确认观察时间，撤单被熔断阻止期间，受阻通知只发一次，首见段恢复后重新受阻才再次提醒。评审修复：回执仅全为成交或其他终态时不再误报撤单成功；撤单受阻且身份与数量事实齐全时显示实际残余份数。验证：三文件 Docker 定向测试616通过；未进行 live 调用、下单、push 或部署。
