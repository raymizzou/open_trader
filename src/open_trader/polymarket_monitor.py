...
class PolymarketMonitor:
    ...
    async def _subscribe(self, client: object) -> None:
        ...
        if refresh_cross_venue:
            try:
                await self._refresh_cross_venue_books(
                    client,
                    cross_venue_tokens,
        ...
        with self._lock:
            ...
            if (
                cross_venue_generation != self._cross_venue_generation
                or set(token_ids) != current_tokens
            ):
                self._subscription_dirty = True
                return
        try:
    ...
    async def _refresh_cross_venue_books(
        self,
        client: object,
    ...
    def _normalize_market(
        ...
        return {
            ...
            "slug": slug,
            "question": question,
            "end_date": _value(value, "end_date", default=None),
            "volume_24h": event_row["volume_24h"],
    ...
    async def _confirm_market(self, client: object, market_row: dict[str, object]) -> dict[str, object] | None:
        ...
        opportunity = {
            ...
            "resolution_at": _timestamp_or_none(market_row.get("end_date")),
