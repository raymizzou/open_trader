...
function renderTrendReportEntry(broker) {
  ...
  const reviewButton = !reviews ? "" : review.available
  ...
  const details = report.available
    ? `<span>报告日期 ${escapeHtml(formatPlain(report.report_date))}</span><span>数据截至 ${escapeHtml(formatPlain(report.data_date))}</span>`
    : `<span>${escapeHtml(formatPlain(report.status_text || "今日暂无趋势报告"))}</span>`;
  const reviewStatus = reviews && !review.available
