...
function predictionObservationCoverage(payload) {
  const coverage = payload?.n_leg_coverage && typeof payload.n_leg_coverage === "object"
    ? payload.n_leg_coverage
    : null;
  const observation = payload?.n_leg_observation && typeof payload.n_leg_observation === "object"
    ? payload.n_leg_observation
    : null;
  if (!coverage && !observation) return "";
  const display = (value, fallback = "—") => predictionHasValue(value) ? String(value) : fallback;
  const count = (...keys) => {
    for (const key of keys) {
      if (predictionHasValue(coverage?.[key])) return String(coverage[key]);
      if (predictionHasValue(observation?.[key])) return String(observation[key]);
    }
    return "—";
  };
  const poolCount = count("pool_count");
  const poolLimit = count("pool_limit");
  const capacity = predictionHasValue(coverage?.capacity)
    ? String(coverage.capacity)
    : poolCount !== "—" && poolLimit !== "—" ? `${poolCount}/${poolLimit}` : "—";
  const latestCount = count("latest_count");
  const freshCount = count("fresh_count");
  const positiveCount = count("positive_count");
  const nonPositiveCount = count("non_positive_count");
  const blockedCount = count("blocked_count");
  const statusKey = String(observation?.status || coverage?.status || "UNKNOWN").toUpperCase();
  const statusLabels = {
    READY: "已就绪",
    EMPTY: "明确为空",
    STALE: "数据过期",
  ...
  const statusLabel = statusLabels[statusKey] || statusKey || "未知";
  const sourceScope = display(
    coverage?.source_scope || observation?.source_scope || "官方来源范围未返回",
  );
  const generation = display(coverage?.generation ?? observation?.generation);
  ...
  const emptyNotice = filteredRows.length
    ? ""
    : statusKey === "UNKNOWN"
      ? `<p class="pm-observation-empty">当前状态未知，尚未取得可确认的观测快照；不会显示为零。</p>`
      : statusKey === "EMPTY"
...
function lpDashboardRate(value) {
...
function lpDashboardObservationReason(reason) {
  const labels = {
    current_yield_unknown: "当前收益率未知",
    reward_rate_unknown: "奖励率未知",
    reward_market_missing: "奖励市场缺失",
    reward_rates_unknown: "奖励数据未知",
    reward_rates_stale: "数据过期",
    reward_qualification_unknown: "计奖资格未知",
    risk_unknown: "风险未知",
    risk_warning: "风险已触线",
    yield_below_threshold: "收益率低于 0.1%／小时",
    trial_baseline_unrecorded: "小单基准未记录",
    no_exposure: "无占用本金",
    account_facts_unknown: "账户数据未知",
    account_snapshot_stale: "账户快照过期",
    account_facts_stale: "账户事实过期",
...
function lpDashboardAccountFactsStale(observation) {
  const reason = String(observation?.reason || "");
  return [
    "account_snapshot_stale",
    "account_facts_unknown",
    "account_facts_stale",
    "observation_stale",
    "exposure_changed",
  ].includes(reason);
...
function lpDashboardObservationDetail(observation, reward, risk) {
  ...
  const baselineText = baseline
    ? `试挂基准 ${escapeHtml(lpDashboardRate(baseline.yield_pct_per_hour))} · ${escapeHtml(lpDashboardMoney(baseline.occupied_capital_usd))} · ${escapeHtml(predictionValue(baseline.quantity, "UNKNOWN"))} 份 × ${escapeHtml(lpDashboardPrice(baseline.price))} · ${escapeHtml(predictionHktTimestamp(baseline.checked_at, "UNKNOWN"))}`
    : `试挂基准未记录 · ${escapeHtml(lpDashboardObservationReason(observation.reason || "trial_baseline_unrecorded"))}`;
  const accountFactsStale = lpDashboardAccountFactsStale(observation);
  const qualified = accountFactsStale
    ? "UNKNOWN"
  ...
  const currentReward = observation.state === "known" && !accountFactsStale
    ...
    : "UNKNOWN";
  const capital = accountFactsStale ? "UNKNOWN" : lpDashboardMoney(observation.occupied_capital_usd);
  const addRoom = !accountFactsStale && observation.add_room && observation.add_room.available === true ? "有" : "无";
  const addRoomReason = addRoom === "无"
    ? lpDashboardObservationReason(observation.add_room?.reason || observation.reason)
    : "当前收益与风险均满足门槛";
  const checkedAt = predictionHasValue(observation.checked_at)
    ? predictionHktTimestamp(observation.checked_at)
    : "UNKNOWN";
  const channelRows = observation.risk_alerts && typeof observation.risk_alerts === "object"
    ? Object.entries(observation.risk_alerts)
      .flatMap(([, alert]) => alert && typeof alert === "object" && alert.channels && typeof alert.channels === "object"
        ? Object.entries(alert.channels).map(([channel, result]) => `${channel}：${result && result.success === true ? "已发送" : result && result.suppressed === true ? "静音" : "未发送"}`)
        : [])
    : [];
  const alertRows = observation.risk_alerts && typeof observation.risk_alerts === "object"
    ? Object.values(observation.risk_alerts)
      .filter((alert) => alert && typeof alert === "object" && (predictionHasValue(alert.title) || predictionHasValue(alert.message)))
      .map((alert) => {
        const title = predictionHasValue(alert.title) ? escapeHtml(alert.title) : "";
        const message = predictionHasValue(alert.message)
          ? escapeHtml(alert.message).replace(/\r?\n/g, "<br>")
          : "";
...
function lpDashboardExposureRiskCell(item, session, observation) {
  ...
  const directions = !accountFactsStale && Array.isArray(observation.risk_directions)
    ? observation.risk_directions.filter((row) => row && typeof row === "object")
    : [];
  const rows = directions.length ? directions : [{
    state: !accountFactsStale && observation.risk_state === "known" ? "known" : "unknown",
    warning: accountFactsStale ? null : observation.risk_warning,
    risk_principal: accountFactsStale ? null : observation.occupied_capital_usd,
    stress_loss: null,
    loss_ratio: null,
    reason_codes: [observation.reason],
  }];
  const riskMarkup = rows.map((direction) => {
    const directionLabel = rows.length > 1
      ? `${escapeHtml(predictionValue(direction.outcome, "UNKNOWN"))} `
      : "";
    const known = direction.state === "known"
      && Number.isFinite(Number(direction.loss_ratio))
      && Number.isFinite(Number(direction.stress_loss));
    if (!known) {
      const reason = Array.isArray(direction.reason_codes) && direction.reason_codes[0]
        ? direction.reason_codes[0]
        : observation.reason || "risk_unknown";
      return `<div><strong>${directionLabel}— · —</strong><span class="sub">${escapeHtml(lpDashboardObservationReason(reason))}</span></div>`;
    }
    const ratio = `${(Number(direction.loss_ratio) * 100).toFixed(2).replace(/\.?0+$/, "")}%`;
    const amount = lpDashboardMoney(direction.stress_loss);
    const status = direction.warning === true ? "已触线" : "未触线";
    const tone = direction.warning === true ? " class=\"pm-tone-danger\"" : "";
    return `<div><strong${tone}>${directionLabel}${escapeHtml(ratio)} · ${escapeHtml(amount)}</strong><span class="sub"${tone}>${status}</span></div>`;
  }).join("");
  return riskMarkup;
}
...
function lpDashboardTodayQuantityCell(orders, session) {
  ...
  const cancelRight = cancellable.length >= 2
    ...
    : orders.length === 1 && cancellable.length === 1
      ...
      : "";
  const headlineRight = (cancelRight || augmentButton)
    ? "<span style=\"display:flex;gap:6px;align-items:center\">" + cancelRight + augmentButton + "</span>"
    : "";
  ...
  if (multi) {
    ...
    markup += orders.map((row) => {
      ...
      if (row?.augment === true) parts.push("加量单");
...
function lpDashboardTodayMarketRow(
  ...
  return "<tr data-lp-today-market=\"" + escapeHtml(identity) + "\""
    ...
    + "<td data-label=\"委托与成交量\">" + lpDashboardTodayQuantityCell(orders, session) + "</td></tr>";
}
...
function lpTrialCandidateRow(row, currentConditionId) {
  ...
  // Issue 158: the title is already the Polymarket link; the action column
  // only hosts the system entry button (no preset gating — the server
  // decides; the read-only page keeps it disabled).
...
function predictionLlmDecisionLabel(opportunity) {
  ...
  const modelText = String(opportunity?.codex_model || "").toLowerCase();
  const provider = modelText.includes("glm")
    ? "智谱 GLM"
    : modelText.includes("deepseek")
      ? "DeepSeek"
      : modelText
        ? "Codex"
        : "LLM";
...
function predictionLpFunnelCondition(conditions, key) {
  ...
  if (value === null || value === undefined || String(value).trim() === "") return "规则不可用（UNKNOWN）";
  if (typeof value === "object") {
    const parts = Object.entries(value).map(([name, item]) => `${name}：${item}`);
    return parts.length ? parts.join(" · ") : "规则不可用（UNKNOWN）";
  }
  return String(value);
}
...
function predictionLpPreparation(preparation) {
  ...
  const itemSummaryProvided = [
    "preparation_item_total", "paused_market_count", "waiting_market_count",
    "retrying_market_count", "failed_market_count",
  ].some((key) => Object.prototype.hasOwnProperty.call(preparation, key));
  const completed = Number(preparation.completed_count);
  const total = Number(preparation.total_count);
  const partialCoverage = rawState === "partial"
    || ["paused_market_count", "waiting_market_count", "retrying_market_count", "failed_market_count"]
...
function predictionFunnelRejections(counts) {
  ...
  const labels = {
    ...
    cost_limit: "成本上限",
    outside_5pct: "距盈亏平衡",
    codex_rejected: "LLM 拒绝",
    codex_unavailable: "LLM 不可用",
    rules_changed: "规则变化",
...
function predictionRelationFunnel(payload) {
  ...
  const zhipuUsage = usageByProvider.zhipu && typeof usageByProvider.zhipu === "object" ? usageByProvider.zhipu : {};
...
function predictionLlmEngineHtml(discovery) {
  const engine = discovery?.llm_provider && typeof discovery.llm_provider === "object" ? discovery.llm_provider : {};
  ...
  const buttons = PREDICTION_LLM_PROVIDERS.map((item) => {
    ...
    return `<button type="button" class="pm-llm-option" data-llm-provider="${item.id}" aria-pressed="${isCurrent}"${disabled ? " disabled" : ""}${!configured ? " title=\"未配置 API Key\"" : ""}>${isCurrent ? PREDICTION_LLM_CHECK_SVG + " " : ""}${escapeHtml(item.name)}${sub ? ` <span class="pm-llm-sub">${escapeHtml(sub)}</span>` : ""}${inFlight ? " 切换中…" : ""}</button>`;
  }).join("");
  const usageChip = (provider, label) => {
    const item = usageByProvider[provider] && typeof usageByProvider[provider] === "object" ? usageByProvider[provider] : {};
    const failing = Number(item.failures) > 0;
...
async function switchPredictionLlmProvider(provider) {
  if (!provider || state.predictionMarket.llmSwitchInFlight) return;
  state.predictionMarket.llmSwitchInFlight = provider;
  renderPredictionMarket();
  try {
    await predictionPost("/api/prediction-arbitrage/llm-provider", {provider});
  } catch (error) {
    state.predictionMarket.error = error instanceof Error ? error.message : String(error);
  } finally {
    state.predictionMarket.llmSwitchInFlight = "";
  }
...
function predictionRelationDiscoveryPanel(payload) {
  ...
  const catalogStatus = predictionFunnelStatus(catalog.status);
  const catalogPillClass = catalogStatus === "正常" ? "action" : "watch";
  const usageLine = (provider, label) => {
    const item = usageByProvider[provider] && typeof usageByProvider[provider] === "object" ? usageByProvider[provider] : {};
    return `${label} ${predictionNumber(item.calls ?? (provider === "codex" ? usage.calls : undefined), "0")} calls · ${predictionNumber(item.failures ?? (provider === "codex" ? usage.failures : undefined), "0")} fail · ${predictionNumber(item.cache_hits ?? (provider === "codex" ? usage.cache_hits : undefined), "0")} cache(本次运行)`;
  };
  return `<details class="pm-panel pm-relation-discovery" aria-label="关联合约扫描"><summary class="pm-panel-heading pm-collapse-summary"><div><h2>关联合约扫描</h2><p>关系目录持久化，成交筛选每分钟刷新。</p></div><span class="pm-pill ${catalogPillClass}">${escapeHtml(catalogStatus)}</span></summary><div class="pm-relation-summary pm-relation-summary-grid"><span>最近全量扫描<strong>${escapeHtml(predictionValue(catalog.completed_at, "未完成"))}</strong></span><span>扫描耗时<strong>${escapeHtml(duration)}</strong></span><span>下次计划<strong>${escapeHtml(next)}</strong></span><span>LLM 24h<strong>${escapeHtml(usageLine("zhipu", "智谱"))}<br>${escapeHtml(usageLine("codex", "Codex"))}<br>${escapeHtml(usageLine("deepseek", "DeepSeek"))}</strong></span></div>${predictionLlmEngineHtml(discovery)}<details class="pm-scan-logs"><summary>扫描日志（${logs.length} 条）</summary><ul>${logRows}</ul></details></details>`;
}
...
function startPredictionPolling() {
  ...
  state.predictionMarket.pollId = window.setInterval(async () => {
    renderPredictionMarket();
    await Promise.all([fetchPredictionVenues(), fetchCurrentPredictionPane()]);
  }, 5000);
}
    ...
    if (state.predictionMarket.payload) {
      ...
      const markStale = (row) => {
        ...
        return {
          ...
          result: {
            ...result,
            status: result.status === "PASS" ? "BLOCKED" : result.status,
            current: false,
            reason: "STATE_FETCH_FAILED",
          },
        };
      };
      const staleLatest = Array.isArray(observation.latest)
        ? observation.latest.map(markStale)
        : observation.latest;
      const staleResults = Array.isArray(observation.results)
        ? observation.results.map(markStale)
        : observation.results;
      const oldCoverage = payload.n_leg_coverage && typeof payload.n_leg_coverage === "object"
        ? payload.n_leg_coverage
        : {};
      const staleResultCount = Array.isArray(staleResults)
        ? staleResults.filter((row) => row && typeof row.result === "object").length
        : 0;
      state.predictionMarket.payload = {
        ...payload,
        stale: true,
        n_leg_coverage: {
          ...oldCoverage,
          status: "ERROR",
          status_reason: "STATE_FETCH_FAILED",
          fresh: 0,
          fresh_count: 0,
          positive: 0,
          positive_count: 0,
          ...
          blocked_count: staleResultCount,
        },
        n_leg_observation: {
          ...observation,
          status: "ERROR",
...
function lpSubmitStateMessage(result) {
  ...
  if (resultState === "locked") {
    if (reason === "circuit_breaker_open") return "交易熔断开启中，禁止新下单；界面保持只读。";
    if (reason === "mutation_blocked") return "系统当前处于只读保护，禁止提交变更；请稍后重试。";
...
function lpOrderIntent(row) {
  const selected = row?.selected_direction && typeof row.selected_direction === "object"
    ? row.selected_direction : {};
  const target = Number(row?.estimated_target_quantity);
  // The raw price string feeds the number input and the case labels; the
  // display-only cent formatting of lpDashboardPrice would corrupt inputs.
  const price = predictionHasValue(selected.price) ? String(selected.price) : "";
...
function lpOrderSwitchCase(mode) {
  ...
  if (!data || !["trial", "five", "custom"].includes(mode)) return;
  const root = elements["prediction-market-modal-root"];
  const priceInput = root.querySelector("#lp-order-price");
  const qtyInput = root.querySelector("#lp-order-quantity");
  if (data.mode === "custom" && priceInput && qtyInput) {
    data.price = priceInput.value;
    data.quantity = qtyInput.value;
  }
  data.mode = mode;
  if (mode === "trial") {
    data.price = data.trialPrice;
    data.quantity = data.trialQuantity;
  } else if (mode === "five" && data.fiveQuantity) {
    data.price = data.trialPrice;
    data.quantity = data.fiveQuantity;
  }
  renderPredictionModal();
}

function lpAugmentSwitchCase(mode) {
  const data = predictionModal.data;
  if (!data || !["five", "custom"].includes(mode)) return;
  const qtyInput = elements["prediction-market-modal-root"].querySelector("#lp-augment-quantity");
  if (data.mode === "custom" && qtyInput) data.quantity = qtyInput.value;
  data.mode = mode;
  if (mode === "five" && data.fiveQuantity) data.quantity = data.fiveQuantity;
  renderPredictionModal();
}

function predictionModalHtml(kind, data = {}) {
  if (kind === "lp_order") return lpOrderModalHtml(data);
  if (kind === "lp_augment") return lpAugmentModalHtml(data);
...
// Issue 158: live cost / merged-A refresh while typing in the LP modals.
function handlePredictionModalInput(event) {
...
function relationExpected(detail) {
...
function relationCatalogItem(item, expanded = false) {
  const endpoints = Array.isArray(item?.endpoints) ? item.endpoints : [];
...
function relationReviewDrawer() {
  ...
  const list = review.loading ? `<p class="pm-relation-empty">正在加载关系目录…</p>` : review.items.length ? review.items.map((item) => {
    const expanded = Boolean(detail && item.version_id === detail.version_id);
    return relationCatalogItem(item, expanded) + (expanded ? relationDetailHtml(detail) : "");
  }).join("") : `<p class="pm-relation-empty">此视图暂无关系。</p>`;
...
function handlePredictionMarketChange(event) {
  ...
  if (directionSelect) {
    ...
    const outcome = String(directionSelect.value || "");
    if (!conditionId || !outcome) return;
    state.predictionMarket.lpDirectionSelections[conditionId] = outcome;
    renderPredictionMarket();
    return;
  }
  const filter = event.target.closest?.("[data-observation-filter]");
  if (!filter) return;
  state.predictionMarket.observationFilter = String(filter.value || "all");
  renderPredictionMarket();
}
...
async function handlePredictionMarketClick(event) {
  ...
  // Issue 158: candidate row 挂单 entry — opens the lp_order intent modal.
  const lpOrderEntry = event.target.closest("[data-action='lp-order-entry']");
  if (lpOrderEntry && !lpOrderEntry.disabled) {
    const conditionId = String(lpOrderEntry.dataset.conditionId || "");
    const outcome = String(lpOrderEntry.dataset.outcome || "");
    const dashboard = state.predictionMarket.lpDashboard;
    const row = lpDashboardRows(dashboard?.candidates).find((item) => {
      if (String(item?.condition_id || "") !== conditionId) return false;
      return !outcome || String(item?.selected_direction?.outcome || "") === outcome;
    });
    if (row) openPredictionModal("lp_order", lpOrderEntry, lpOrderIntent(row));
    return;
  }
  // Issue 158: today-table 加量 entry — only rendered on session rows.
  ...
  if (lpAugmentEntry && !lpAugmentEntry.disabled) {
    ...
    if (group && session) {
      openPredictionModal("lp_augment", lpAugmentEntry, lpAugmentIntent(group.orders, session));
    }
    return;
  }
  ...
  if (relationRow) {
    const review = state.predictionMarket.relationReview;
    if (relationRow.dataset.relationVersionId === review.detail?.version_id) {
      review.detail = null;
      renderPredictionMarket();
      return;
    }
...
async function handlePredictionModalClick(event) {
  ...
  // Issue 158: case switches are instant UI-only re-renders.
  if (action === "lp-order-case") {
    lpOrderSwitchCase(String(event.target.closest("[data-modal-action]")?.dataset.caseMode || ""));
    return;
  }
  if (action === "lp-augment-case") {
