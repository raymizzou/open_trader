...
def size_entry_by_risk(
    *,
    ...
    portfolio_remaining_risk: Decimal | None,
    ...
    ) or not protection_line.is_finite() or (
        portfolio_remaining_risk is not None
        and (
            not portfolio_remaining_risk.is_finite()
            or portfolio_remaining_risk < 0
        )
    ) or any(
        not value.is_finite() or value < 0
        for value in (available_cash, normal_cost_rate)
    ):
    ...
    if portfolio_remaining_risk is not None:
        caps.append(("组合剩余风险", portfolio_remaining_risk / unit_risk))
    caps.append(("现金", available_cash / cash_per_unit))
    ...
    # A single lot always stays listed even when every cap is below one lot;
    # the caller surfaces the over-limit constraints as extra-risk notes.
    exceeded_limits = tuple(name for name, cap in caps if cap < lot_size)
    if quantity <= 0:
        quantity = lot_size
    ...
    return EntryRiskResult(
        ...
        reason=(
            "；".join(f"一手超过{name}" for name in exceeded_limits)
            if exceeded_limits
            else ""
        ),
