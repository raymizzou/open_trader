...
def _source_file_state(source_path: Path) -> tuple[tuple[bool, int, int], ...]:
    ...
    for path in (source_path, Path(f"{source_path}-wal")):
        try:
            stat = path.stat()
    ...
    *, source_data_dir: Path, shadow_data_dir: Path
    ...
    for _ in range(_SNAPSHOT_RETRIES):
        ...
        try:
            ...
            source.close()
        if before == _source_file_state(source_path):
            break
    else:
        raise RuntimeError("source database changed while reading shadow snapshot")
